...
@property (nonatomic, assign) BOOL showingDFP;
...
- (void)setAdNetworks:(NSArray *)adNetworks {
  ...
  _preferredAds = (CJPAdNetwork) [_adNetworks[0] intValue];
}
...
- (void)startWithViewController:(UIViewController *)contentController {
  ...
  _isTabBar = [_contentController isKindOfClass:[UITabBarController class]];
  _isNavController = [_contentController isKindOfClass:[UINavigationController class]];
...
- (void)createBanner:(NSNumber *)adID {
  ...
  if (adType == CJPAdNetworkiAd) {
    ...
    if (_adMobTestDeviceIDs != nil) {
      adMobRequest.testDevices = _adMobTestDeviceIDs;
    }
    ...
    // create DoubleClick for Publishers
  else if (adType == CJPAdNetworkDFP) {
    CJPLog(@"Creating DoubleclickForPublishers");
    GADAdSize adMobSize;
    if (_useDFPSmartSize) {
      adMobSize = isPortrait ? kGADAdSizeSmartBannerPortrait : kGADAdSizeSmartBannerLandscape;
      _dfpView = [[DFPBannerView alloc] initWithAdSize:adMobSize];
    }
    else {
      // Legacy AdMob ad sizes don't fill the full width of the device screen apart from iPhone when in portrait view
      // We need to offset the x position so the ad appears centered - Calculation: (View width - Ad width) / 2
      ...
      CGSize cgAdMobSize = CGSizeFromGADAdSize(adMobSize);
      CGFloat adMobXOffset = (screenWidth - cgAdMobSize.width) / 2;
      _dfpView = [[DFPBannerView alloc] initWithFrame:CGRectMake(adMobXOffset, self.view.frame.size.height - cgAdMobSize.height, cgAdMobSize.width, cgAdMobSize.height)];
    }

    // Specify the ad's "unit identifier." This is your AdMob Publisher ID.
    _dfpView.adUnitID = _dfpUnitID;

    // Set initial frame to be off screen
    CGRect bannerFrame = _dfpView.frame;
    if (_adPosition == CJPAdPositionBottom) {
      bannerFrame.origin.y = [[UIScreen mainScreen] bounds].size.height;
    }
    else if (_adPosition == CJPAdPositionTop) {
      bannerFrame.origin.y = 0 - _dfpView.frame.size.height;
    }
    _dfpView.frame = bannerFrame;

    // Let the runtime know which UIViewController to restore after taking
    // the user wherever the ad goes and add it to the view hierarchy.
    _dfpView.rootViewController = self;
    _dfpView.delegate = self;
    _dfpView.hidden = YES;
    [_containerView insertSubview:_dfpView atIndex:0];

    // Request an ad
    DFPRequest *dfpRequest = [DFPRequest request];

    // Device identifier strings that will receive test AdMob ads
    if (_dfpTestDeviceIDs != nil) {
      dfpRequest.testDevices = _dfpTestDeviceIDs;
    }
    ...
    //
    if ([_tagForChildDirectedTreatment isEqualToString:@"0"] || [_tagForChildDirectedTreatment isEqualToString:@"1"]) {
      BOOL tagForCOPPA = [_tagForChildDirectedTreatment isEqualToString:@"1"];
      [dfpRequest tagForChildDirectedTreatment:tagForCOPPA];
      CJPLog(@"COPPA has been set to %i", tagForCOPPA);
    }
    ...
    // Gender
    if (_adMobGender != kGADGenderUnknown) {
      dfpRequest.gender = _adMobGender;
      CJPLog(@"AdMob targeting: Gender has been set to %i", (int) _adMobGender);
    }

    // Birthday
    if (_adMobBirthday != nil) {
      dfpRequest.birthday = _adMobBirthday;
      CJPLog(@"AdMob targeting: Birthday has been set.");
    }

    // Location
    if (_adMobUserLocation != nil) {
      [dfpRequest setLocationWithLatitude:[_adMobUserLocation[@"latitude"] floatValue] longitude:[_adMobUserLocation[@"longitude"] floatValue] accuracy:[_adMobUserLocation[@"accuracy"] floatValue]];
      CJPLog(@"AdMob targeting: Location has been set to %@", _adMobUserLocation);
    }
    else if (_adMobLocationDescription != nil) {
      [dfpRequest setLocationWithDescription:_adMobLocationDescription];
      CJPLog(@"AdMob targeting: Location has been set to \"%@\"", _adMobLocationDescription);
    }


    // Now we can load the requested ad
    [_dfpView loadRequest:dfpRequest];
    CJPLog(@"Added DFP to view and requested ad.");
  }
}
...
- (void)removeBanner:(NSNumber *)adID permanently:(BOOL)permanent {
  ...
  if (adType == CJPAdNetworkiAd) {
    ...
    if (permanent && !_iAdView.bannerViewActionInProgress) {
  ...
  if (adType == CJPAdNetworkDFP) {
    ...
    _dfpView.frame = bannerFrame;
    [_containerView sendSubviewToBack:_dfpView];
    if (permanent) {
      _dfpView.delegate = nil;
      [_dfpView removeFromSuperview];
      _dfpView = nil;
      CJPLog(@"Permanently removed DFP from view.");
    }
    else {
      CJPLog(@"Temporarily hiding DFP off screen.");
    }
  }
...
- (void)removeAdsAndMakePermanent:(BOOL)permanent andRemember:(BOOL)remember {
  ...
  if (_dfpView != nil) [self removeBanner:@(CJPAdNetworkDFP) permanently:permanent];
...
- (void)restartAdsAfterDelay:(NSTimeInterval)delay {
  ...
  if (!_adsRemoved && !_showingiAd && !_showingAdMob && !_showingDFP) {
...
- (void)viewDidLayoutSubviews {
  ...
  BOOL preiOS7 = [[[UIDevice currentDevice] systemVersion] floatValue] < 7;
  UIView *tbcView = nil;
  UIView *tbcTabs = nil;
  ...
  if (_isTabBar && _aboveTabBar && _adPosition == CJPAdPositionBottom) {
    tbcView = _contentController.view.subviews[0];
    tbcTabs = _contentController.view.subviews[1];
  ...
  if (_iAdView || _adMobView || _dfpView) {
    // iAd specific stuff
    if (_iAdView) {
    ...
    if (_dfpView) {
      adType = CJPAdNetworkAdMob;
      if (_useDFPSmartSize) {
        _dfpView.adSize = isPortrait ? kGADAdSizeSmartBannerPortrait : kGADAdSizeSmartBannerLandscape;
        bannerFrame = _dfpView.frame;
      }
      else {
        // Legacy AdMob doesn't have different orientation sizes - we just need to change the X offset so the ad remains centered
        bannerFrame = _dfpView.frame;
    ...
    if (_showingiAd || _showingAdMob || _showingDFP) {
    ...
    else if (_showingDFP) _dfpView.frame = bannerFrame;
  }
  ...
  if (!preiOS7 && !_showingiAd && !_showingAdMob && !_showingDFP && _adPosition == CJPAdPositionTop) {
...
- (void)bannerViewDidLoadAd:(ADBannerView *)banner {
  ...
  if (!_showingiAd) {
    ...
    if (_showingAdMob || _adMobView != nil) {
      ...
      [self removeBanner:@(CJPAdNetworkAdMob) permanently:_preferredAds == CJPAdNetworkiAd];
      _showingAdMob = NO;
    }
    // Ensure DFP is hidden
    if (_showingDFP || _dfpView != nil) {
      // If we're preferring iAd then we should remove AdMob rather than simply hiding it
      [self removeBanner:@(CJPAdNetworkDFP) permanently:_preferredAds == CJPAdNetworkiAd];
      _showingDFP = NO;
    }
  }
  _showingiAd = YES;
...
- (void)adViewDidReceiveAd:(GADBannerView *)bannerView {
  ...
  if ([[[bannerView class] description] isEqualToString:[[GADBannerView class] description]]) {
    ...
    if (!_showingAdMob) {
      ...
      // Ensure DFP is hidden, then show AdMob
      if (_showingDFP || _dfpView != nil) {
        // If we're preferring AdMob then we should remove DFP rather than simply hiding it
        if (_preferredAds == CJPAdNetworkAdMob) {
          [self removeBanner:@(CJPAdNetworkDFP) permanently:YES];
        }
        else if (_iAdView.isBannerLoaded) {
          [self removeBanner:@(CJPAdNetworkDFP) permanently:NO];
        }
        _showingDFP = NO;
      }
  ...
  // handle dfp
  if ([[[bannerView class] description] isEqualToString:[[DFPBannerView class] description]]) {
    CJPLog(@"New DFP ad received.");
    if (!_showingDFP) {
      // Ensure iAd is hidden, then show DFP
      if (_showingiAd || _iAdView != nil) {
        // If we're preferring AdMob then we should remove iAd rather than simply hiding it
        if (_preferredAds == CJPAdNetworkAdMob) {
          [self removeBanner:@(CJPAdNetworkiAd) permanently:YES];
      ...
      // Ensure AdMob is hidden, then show DFP
      if (_showingAdMob || _adMobView != nil) {
        // If we're preferring DFP then we should remove AdMob rather than simply hiding it
        if (_preferredAds == CJPAdNetworkDFP) {
    ...
    [UIView animateWithDuration:0.25
                     ...
                     completion:^(BOOL finished) {
                       // Ensure view isn't behind the container and untappable
                       if (finished) [_containerView bringSubviewToFront:_dfpView];
                     }];
  }
...
- (void)adView:(GADBannerView *)view didFailToReceiveAdWithError:(GADRequestError *)error {
  // handle AdMob
  if ([[[view class] description] isEqualToString:[[GADBannerView class] description]]) {
    // Ensure view is hidden off screen
  ...
  // handle DFP
  if ([[[view class] description] isEqualToString:[[DFPBannerView class] description]]) {
    // Ensure view is hidden off screen
    if (_dfpView.frame.origin.y >= 0 && _dfpView.frame.origin.y < _containerView.frame.size.height) {
      [self removeBanner:@(CJPAdNetworkDFP) permanently:NO];
    }
    _showingDFP = NO;

    CJPLog(@"Failed to receive DFP. %@", error.localizedDescription);
